...
function makeProvider(subscriptions) {
  ...
  const automaticJump = (textEditor, { start, end }: Range) => {
    ...
    const options = {
      jumpToImport: atom.config.get("lp-js-hyperclick.jumpToImport"),
    }
  ...
  const followSuggestionPath = (fromFile, suggestion) => {
    const resolveOptions = {
      extensions: atom.config.get("lp-js-hyperclick.extensions"),
    }
    ...
    if (resolved.type === "url") {
      ...
      const options = {
        pending: atom.config.get("lp-js-hyperclick.usePendingPanes"),
      }
...
module.exports = {
  ...
  activate() {
    ...
    if (!atom.packages.isPackageLoaded("nuclide")) {
      require("atom-package-deps").install("lp-js-hyperclick").then(() => {})
    }
